#!/usr/bin/env bash
#SBATCH -J rlinf_ctrl_world_pi05_rl
#SBATCH -A naiss2025-22-1173
#SBATCH --output=/mimer/NOBACKUP/groups/naiss2024-5-164/Hanzhi/RLinf/logs/ctrl_world_pi05_rl_out_%j.txt
#SBATCH --error=/mimer/NOBACKUP/groups/naiss2024-5-164/Hanzhi/RLinf/logs/ctrl_world_pi05_rl_err_%j.txt
#SBATCH --nodes=2
...
if [[ -f "${SLURM_SUBMIT_DIR}/examples/embodiment/train_embodied_agent.py" ]]; then
  REPO_PATH="${SLURM_SUBMIT_DIR}"
elif [[ -f "${SLURM_SUBMIT_DIR}/RLinf/examples/embodiment/train_embodied_agent.py" ]]; then
...
# Use a short runtime Ray temp dir to avoid AF_UNIX socket path-length limit (107).
# Persist logs by syncing runtime Ray logs to LOG_DIR at job exit.
...
RAY_DONE_FILE="${LOG_DIR}/.ray_training_done"
...
printf -v TRAIN_CMD_STR '%q ' "${CMD[@]}"
export TRAIN_CMD_STR
...
# Run head/worker bootstrap and training in a single Slurm step.
